...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<PersonInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/people` : null),
    () => getPeople({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Person',
      path: isTenantUser ? `/people/view/${data?.[0]?.id}` : '/people',
      entity: 'person',
      service: AccessServiceEnum.PROJECT,
      icon: FiUserCheck,
    },
    { name: 'Expense', path: '/expenses', entity: 'expense', service: AccessServiceEnum.PROJECT, icon: FiDollarSign },
